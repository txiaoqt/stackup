...
export default function SignInView({ onSignIn, setScreen }: SignInViewProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (password.length < 4) {
      setErrorSim("Password validation parameter is too short.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#EEF2FF] text-[#1A2260] flex flex-col justify-between p-6 relative overflow-hidden select-none">
      ...
      <div className="max-w-md w-full bg-white border border-[#C7D2FE] rounded-2xl p-7 mx-auto my-auto z-10 shadow-[0_24px_70px_rgba(26,34,96,0.14)] space-y-6">
        <div>
          ...
          <p className="text-sm text-[#5B668F] mt-1.5">
            Connect your workspace pipeline variables database.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {isSubmitting ? "Authenticating..." : "Access Control Cabin"}
          </button>
        </form>
        ...
        <div className="text-center pt-1">
          <span className="text-sm text-[#5B668F]">Don't have an operating center? </span>
          <button
